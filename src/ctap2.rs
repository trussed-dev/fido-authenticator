...
impl<UP: UserPresence, T: TrussedRequirements> Authenticator for crate::Authenticator<UP, T> {
    ...
    fn get_info(&mut self) -> ctap2::get_info::Response {
        ...
        //   0x04 enableLongTouchForReset (CTAP 2.3 §6.11.5; hard-wired on)
    ...
    fn make_credential_into(
            ...
            hmac_secret_mc_input.as_ref()
        {
            ...
            let shared_secret = match pin_protocol_impl.shared_secret(&hmac_secret.key_agreement) {
                Ok(s) => s,
                Err(e) => {
            ...
            if let Err(e) = pin_protocol_impl.verify_pin_auth(
                &shared_secret,
