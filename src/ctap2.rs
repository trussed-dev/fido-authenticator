...
impl<UP: UserPresence, T: TrussedRequirements> Authenticator for crate::Authenticator<UP, T> {
    ...
    fn make_credential_into(
            ...
            hmac_secret_mc_input.as_ref()
        {
            ...
            let shared_secret = pin_protocol_impl.shared_secret(&hmac_secret.key_agreement)?;
            pin_protocol_impl.verify_pin_auth(
                &shared_secret,
            ...
            if salts.len() != 32 && salts.len() != 64 {
                debug_now!("invalid hmac-secret-mc salt length");
            ...
            Some(Bytes::try_from(&*output_enc).map_err(|_| Error::Other)?)
        } else {
            None
        };
        ...
        let authenticator_data = ctap2::make_credential::AuthenticatorData {
            ...
            extensions: {
                    ...
                    || hmac_secret_mc_output.is_some()
                {
                    ...
                    if let Some(out) = hmac_secret_mc_output.clone() {
        ...
        if is_rk {
            ...
            if large_blob_key_requested {
                ...
                response.large_blob_key = match credential {
                    Credential::Stripped(stripped) => stripped.large_blob_key.clone(),
                    Credential::Full(full) => full.data.large_blob_key.clone(),
                };
            }
        }
